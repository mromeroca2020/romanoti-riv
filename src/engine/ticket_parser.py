...
class TicketParser:
    ...
    def _extract_site_code(self, description):
        match = re.search(r"\b[a-z]{2}\d{1,2}\b", description.lower())
        if match:
    ...
    def _extract_rack(self, description):
        ...
        match = re.search(r"\brack\s+([a-z]\d{1,2})\b", text)
        if match:
    ...
    def _extract_named_rack(self, description, kind):
        ...
        patterns = {
            "source": [
                r"\bsource rack\s+([a-z]\d{1,2})\b",
            ],
            "destination": [
                r"\bdestination rack\s+([a-z]\d{1,2})\b",
            ],
        }
    ...
    def _extract_named_ru(self, description, kind):
        ...
        patterns = {
            "source": [
                r"\bru source\s+(ru\d{1,2})\b",
            ],
            "destination": [
                r"\bru destination\s+(ru\d{1,2})\b",
            ],
        }
    ...
    def _extract_named_device(self, description, kind):
        ...
        patterns = {
            "source": [
                r"\bfrom\s+([a-z]{3,}\d+[a-z0-9-]*)\b",
                r"\bsource device\s+([a-z]{3,}\d+[a-z0-9-]*)\b",
            ],
            "destination": [
                r"\bto\s+([a-z]{3,}\d+[a-z0-9-]*)\b",
                r"\bdestination device\s+([a-z]{3,}\d+[a-z0-9-]*)\b",
            ],
        }
        ...
        if kind == "source":
            generic = re.findall(r"\b[a-z]{3,}\d+[a-z0-9-]*\b", text)
            ignored = {"sctask", "screq", "ritm", "inc", "chg", "task"}
            for item in generic:
                if not any(item.startswith(prefix) for prefix in ignored):
                    return item.upper()
    ...
    def _detect_redundancy(self, text):
        return any(term in text for term in ["redundant", "a/b", "dual feed", "primary", "secondary"])
